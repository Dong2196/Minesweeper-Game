...
/** Campo do formulário com a pontuacao. */
...
function principal() {
    ...
    if (varUrl.includes("personalizavel")) {
        risco = obtemRisco()
        tempoPredefinido = obtemTempoLimite()
        console.log(tempoPredefinido)
    }
 ...
 * 
 * @param {string} jogo - Objeto com os dados do jogo.
 */
...
/** 
 * Função que  o recebe  risco da primeira jogada
 
 */
function obtemRisco() {
    let definiçoes = JSON.parse(localStorage.getItem("definiçoes"))
    let risco1 = definiçoes[0].risco
    console.log(risco1)
...
let tempoMax = tempoPredefinido * 60


function mostraTempoRestante() {
    tempoMax = tempoPredefinido * 60
...
function colocaBombas(celula) {
    ...
    /** Identificador do url da página atual do modo de jogo. */
    let varUrl = window.location.href;
    //SAFE PLAY - Verifica se no primeiro click existe bomba e se é o modo fácil

    if (varUrl.includes("facil") || risco === "Sim") {
        if (celulas[y][x].classList.contains("bomba")) {
            celulas[y][x].classList.remove("bomba");
            celulas[y][x].classList.add("livre");
            let count = 0
            console.log("trocou")
            first:
                for (let linha = 0; linha < altura; linha++) {
                    for (let coluna = 0; coluna < comprimento; coluna++) {
                        if (celulas[linha][coluna].classList.contains("livre")) {
                            count++
                            if (count == 2) {
                                celulas[linha][coluna].classList.remove("livre");
                                celulas[linha][coluna].classList.add("bomba");
                                break first;
...
function terminaJogo(celula) {
    ...
    document.getElementById(SPAN_RESULTADO).innerHTML = 'Boom! Game Over!';
    fim = true;

    setTimeout(() => {
        location.href = "../templates/scores.html"
    }, 2500);
}
/* ------------------------------------------------------------------------------------------------------ */
/**
...
function ganhaJogo() {
    ...
    if (count === comprimento * altura - numeroBombas) {
        ...
        document.getElementById(SPAN_RESULTADO).innerHTML = 'PARABÉNS! VENCESTE! 👑 ';
        fim = true;
        trataGravarJogo()
        setTimeout(() => {
            location.href = "../templates/scores.html"
...
/* ------------------------------------------------------------------------------------------------------ */
/** Esta função calcula o tamanho do tabuleiro personalizavel. */
